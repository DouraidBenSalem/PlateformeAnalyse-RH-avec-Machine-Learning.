...
#OUMA : Variables pour la prédiction de performance
scaler_perf = None
pca_perf = None
model_perf = None
...
# ============================================================
#OUMA : CHARGEMENT DU MODÈLE DE PERFORMANCE
# ============================================================
try:

    scaler_perf = joblib.load('PickleFiles/pred_ouma/scaler.pkl')
    print("✅ Scaler performance chargé depuis pred_ouma/scaler.pkl")

    pca_perf = joblib.load('PickleFiles/pred_ouma/pca_model.pkl')
    print("✅ PCA performance chargée depuis pred_ouma/pca_model.pkl")

    model_perf = joblib.load('PickleFiles/pred_ouma/model_xgb.pkl')
    print("✅ Modèle XGBoost performance chargé depuis pred_ouma/model_xgb.pkl")

except Exception as e:
    print(f"❌ Erreur lors du chargement du modèle de performance: {e}")
    scaler_perf = None
    pca_perf = None
    model_perf = None
...
# ============================================================
#OUMA: FONCTION DE PRÉTRAITEMENT POUR PERFORMANCE
# ============================================================

def predict_performance_from_form(form):
    """Transforme les champs du formulaire en features et renvoie la prédiction de performance."""
    if model_perf is None or scaler_perf is None or pca_perf is None:
        raise RuntimeError("Modèle de performance non chargé")

    # ⬇️ Adapte ici avec EXACTEMENT les mêmes noms de champs que dans ton ancien app_pred_per.py
    percent_hike = float(form['percent_salary_hike'])
    job_satisfaction = int(form['job_satisfaction'])
    job_involvement = int(form['job_involvement'])
    years_in_role = int(form['years_in_current_role'])
    years_since_last_promotion = int(form['years_since_last_promotion'])

    # Même feature engineering que dans ton app_pred_per.py
    X = pd.DataFrame([{
    ...
    # Même pipeline que dans ton app_pred_per.py
    X_scaled = scaler_perf.transform(X)
    X_pca = pca_perf.transform(X_scaled)
    raw_pred = model_perf.predict(X_pca)[0]

    # Si ton modèle prédit 0..3 et que tu voulais 1..4
    prediction = int(raw_pred) + 1
    ...
    # On renvoie ce qu'on veut passer au template
    return {
...
# ============================================================
# OUMA :ROUTES PERFORMANCE
# ============================================================

@app.route('/performance')
def performance_page():
    """Page de prédiction de performance employé"""
    # Page avec ton formulaire de performance
    return render_template('index_pred_perf.html')


@app.route('/predict_performance', methods=['POST'])
def predict_performance():
    """Effectue la prédiction de performance employé"""
    try:
        result = predict_performance_from_form(request.form)

        # On récupère la valeur de prédiction et on calcule un booléen
        prediction_value = int(result.get('prediction', 0))
        is_low_perf = prediction_value <= 2   # 1 ou 2 = faible/moyen

        return render_template(
            'result_pred_perf.html',
            **result,
        ...
        traceback.print_exc()
        return jsonify({
            "error": f"Erreur lors de la prédiction de performance: {str(e)}"
        }), 400
